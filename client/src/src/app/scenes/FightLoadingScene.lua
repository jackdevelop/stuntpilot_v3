...
--	audio.playBackgroundMusic(self.currentSceneName_());
    ...
    self.steps[#self.steps + 1] = function()
       --加载数据
       local level = 1000 --param.level;--地图
...
function FightLoadingScene:tick()
    ...
    elseif self.stepIndex >= count then
        ...
        self:performWithDelay(function()
        	local sceneName,backScaneName,isPhysicsWorld = SceneConstants.FightScene()
        	app:enterScene(sceneName,backScaneName,isPhysicsWorld,self.param_)
        end, 0.1)
    end
end
