...
@param isPhysicsWorld 是否是物理场景
]]
function Jscene(sceneName,backSceneName,isPhysicsWorld)
	return function (idx) 
		if idx=="sceneName" then return sceneName
		elseif idx=="backSceneName" then return backSceneName
		elseif idex == "isPhysicsWorld" then return isPhysicsWorld
		else return sceneName,backSceneName,isPhysicsWorld end
	end
end
